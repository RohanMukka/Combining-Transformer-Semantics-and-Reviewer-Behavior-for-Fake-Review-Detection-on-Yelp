...
The text branch leverages a dense representation of the review's content features. In the ReviewGuard architecture, this branch utilizes a fine-tuned RoBERTa-base encoder to produce 768-dimensional embeddings from review text. To maintain computational efficiency and architectural symmetry with the behavior branch, these high-dimensional embeddings are projected via PCA to a 16-dimensional latent representation that captures the principal semantic variance of the review content.

Each review is tokenized with the RoBERTa byte-pair encoding tokenizer, truncated or padded to a maximum sequence length of 256 tokens, and the [CLS] token embedding from the final encoder layer serves as the text representation. Fine-tuning uses the AdamW optimizer~\cite{loshchilov2019decoupled} with a learning rate of $2 \times 10^{-5}$, linear warmup over 10\% of training steps, and cosine decay thereafter. This end-to-end transformer-based pipeline ensures that deep semantic signals are captured before being fused with behavioral data.
...
The RoBERTa-derived text vector and the behavioral feature vector are concatenated to form a fused representation. This yields a 32-dimensional input (16 semantic PCA components + 16 behavior dimensions). This vector is passed through a two-layer MLP classifier: a first hidden layer of 256 units with ReLU activation and dropout rate 0.3, followed by a second hidden layer of 64 units with ReLU activation and dropout rate 0.3, and a final linear output layer with sigmoid activation producing the probability estimate $P(\text{fake} \mid t, b)$. This 32-dimensional fused representation enables efficient SHAP-based explainability while maintaining the deep semantic signals captured by the transformer branch.
