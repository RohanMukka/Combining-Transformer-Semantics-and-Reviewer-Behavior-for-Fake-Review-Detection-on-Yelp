...
features. We show that reading is only half right. Business-level quantities
divide into three kinds that the conventional protocol conflates: statistics
derived from labels, which are unambiguous leakage; the identity of a business
known to have been targeted, which a business-disjoint split correctly removes;
and label-free profiles of a business's own reviews, which a platform observes
for a brand-new listing and which transfer to businesses never seen during
training at 0.805 AUC. Removing the third along with the first two is not
rigour but self-harm, and it is what a strict training-only featurization does.
We therefore evaluate along two axes --- the split and the source of a feature's
statistics --- and report both. Graph models make the distinction unavoidable:
message passing reconstructs a business profile from a node's same-business
neighbours regardless of what its feature matrix was allowed to contain, so a
business-disjoint split is necessary but not sufficient for them. A relation-aware
GNN is matched by a plain MLP given only two graph degrees, placing its advantage
in structural bookkeeping rather than relational learning. We additionally
document two integrity failures in a widely circulated distribution of the
dataset: label-contaminated timestamps yielding 0.965 AUC from the timestamp
alone, and a behavioral feature file uncorrelated with both the label and its
...
\item ReviewGuard, a two-branch detector fusing lexical text features with
      ...
      severe class imbalance, with per-model threshold tuning so that baselines
      are not handicapped (Section~\ref{sec:method}).
...
The difference is substantial, and it means the collapse in
Table~\ref{tab:protocol} cannot be attributed to leakage alone. Part of it is
leakage --- business identity really is unavailable for an unseen business ---
and part is a self-inflicted wound from a featurisation strict enough to discard
the transferable business profiles of Section~\ref{sec:integrity}. Separating
the two is the point of the second axis, and it is the correction we consider
the most useful contribution of this paper: a protocol that removes information
along with leakage will make any behavioral method look worse than it is, and
will do so invisibly.
...
Under business-disjoint evaluation ReviewGuard reaches 0.730 AUC-ROC in the
inductive regime, ahead of both of its ablations, though by a margin that five
folds cannot establish as significant.
