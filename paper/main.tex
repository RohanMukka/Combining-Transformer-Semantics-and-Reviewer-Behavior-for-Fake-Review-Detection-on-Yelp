...
accuracies on it are high. We show that a large share of that performance does
not come from detecting deception. Fake reviews in YelpCHI are concentrated by
business: 657 of its 1{,}224 businesses contain no fake reviews at all, and the
per-business fake rate is by itself a 0.951-AUC predictor of the label. Any
evaluation that allows a business to appear in both training and test folds is
therefore partly measuring memorization of which businesses were spam targets.
We build ReviewGuard, a two-branch detector that fuses a lexical text
representation of the review with reviewer- and business-level behavioral
features, and evaluate it under two protocols that differ only in how folds are
partitioned. Under a reviewer-disjoint split --- the convention in much of this
literature --- a behavioral random forest reaches 0.934 AUC-ROC. Under a
business-disjoint split, with identical features, models, and tuning, the same
model falls to 0.646. The size of each model's drop tracks how much access it
has to business identity: 0.032 for a text-only model, 0.183 for a
behavior-only one, and 0.289 for the random forest. We further document two
integrity failures in a widely circulated distribution of the dataset: the
released timestamps are label-contaminated, yielding 0.965 AUC from the
timestamp alone, and an accompanying behavioral feature file is uncorrelated
with both the label and its own column definitions. Under business-disjoint
evaluation ReviewGuard reaches 0.730 AUC-ROC, ahead of both single branches on
five of five and four of five folds respectively --- a real but modest fusion
benefit at an absolute performance level far below what this benchmark is
usually reported to support. We argue that business-disjoint evaluation should
be reported alongside any YelpCHI result.
\end{abstract}
...
\begin{enumerate}
\item A quantification of business-identity leakage in YelpCHI, and a
      demonstration that it accounts for the majority of the apparent
      performance of behavioral fake review detectors on this benchmark
...
\item A data integrity audit of a widely circulated CSV distribution of
      ...
      behavioral feature file (Section~\ref{sec:integrity}).
...
The same distribution ships a \texttt{behavioral\_features.csv} with six
precomputed columns (average star rating, review count, burst ratio, rating
deviation, category diversity, account age). Reconstructing each column from the
review table it purports to summarize --- for instance, computing each
reviewer's actual mean rating --- yields correlations of at most 0.005 with the
shipped values. The columns also correlate with the label at most 0.008 in
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/business_label_concentration.png}
\caption{Fake reviews are concentrated by business. Left: distribution of
per-business fake rates. Right: businesses sorted by fake rate, showing that
the label is near-constant within a business.}
...
for ReviewGuard on a representative business-disjoint fold, at the
validation-tuned threshold.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/roc_all_models.png}
\caption{ROC curves for ReviewGuard and its two ablations on a
business-disjoint fold.}
\label{fig:roc}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\columnwidth]{figures/cm_reviewguard_fusion.png}
\caption{ReviewGuard confusion matrix on a business-disjoint fold at the
validation-tuned threshold.}
\label{fig:cm}
\end{figure}
