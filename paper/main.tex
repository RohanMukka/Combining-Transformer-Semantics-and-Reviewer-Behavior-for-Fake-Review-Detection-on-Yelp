...
\SI{80.1}{\percent} of them sit in the training set, and under a
business-disjoint split \SI{0.0}{\percent} do. Second, what the resulting
...
Table~\ref{tab:graph} reports the consequence. The relation-aware GNN reaches
0.816 AUC under the business-disjoint split, well above the 0.706 of the same
behavioral features without a graph. Handing those two degrees to a plain MLP
recovers 0.788 of that --- roughly three quarters of the difference --- and
R-S-R degree on its own (0.708) matches what the entire 19-feature inductive set
achieves.

We read the remainder as the honest size of the relational contribution. The
larger part of the GNN's advantage is a count that the business-disjoint split
was intended to withhold and that a feature model can simply be handed; the
smaller part is not, and we do not claim message passing buys nothing here.
